...
###################################################################lineplot##########
	...
	def update(choose):
		if (choose=='edad'):
			...
			fig2 = px.scatter(df_line, x="fecha_compra", y='ventas_promedio', color='edad',hover_data=['cantidad_compras'])
		else:
			df_line=get_views.get_view_by_name('count_avg')
			fig2 = px.scatter(df_line, x="fecha_compra", y='ventas_promedio', color='canal',hover_data=['cantidad_compras'])
		return fig2
...
################################################################ barplot #######
	...
	def barp(select):
		if (select=='canal'):
			...
			df_bar=df_bar.sort_values('volumen_pesos',ascending=False).head(50)
			fig5=px.bar(df_bar, y='grupo_articulo', x="volumen_pesos", color='grupo_articulo', orientation="h", hover_name="ciudad_tienda" )
		return fig5
